...
async function findManyWithRelations(em: EntityManager) {
  return em.find(
    User,
    {},
    { strategy: 'select-in', populate: ['orderCollection.orderLineCollection'] }
  )
}
...
async function main() {
  ...
  results.push(await wrapAndMeasure('mikro-orm-selectin-find-many', findMany))
  results.push(
    await wrapAndMeasure(
      'mikro-orm-selectin-find-many-with-relations',
      findManyWithRelations
    )
  )
  results.push(
    await wrapAndMeasure(
      'mikro-orm-selectin-find-many-with-relations-filter-and-pagination',
      findManyWithRelationsFilterAndPagination
    )
  )
  results.push(
    await wrapAndMeasure(
      'mikro-orm-selectin-find-many-with-nested-where',
      findManyWithNestedWhere
    )
  )
  results.push(
    await wrapAndMeasure(
      'mikro-orm-selectin-find-many-with-nested-where-select-and-pagination',
